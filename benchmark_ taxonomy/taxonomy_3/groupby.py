...
def grouped_summary():
    ...
    summary = df.groupby("sex")["actual_births"].agg(["mean", "sum", "count"])
    ...
    source_lines = inspect.getsource(grouped_summary).split('\n')
    total_lines = len(source_lines)
    core_lines = len([line for line in source_lines if line.strip() and not line.strip().startswith("#")])
    print(f"Total Lines of Code: {total_lines}")
    print(f"Core Lines of Code: {core_lines - 3}")

    return wall_runtime

if __name__ == "__main__":
    mem = memory_usage((grouped_summary,), max_iterations=1)
